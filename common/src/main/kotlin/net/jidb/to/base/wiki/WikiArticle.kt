...
class WikiArticle(val identifier: ResourceLocation, val name: Component, val flavor: Component, val content: WikiContent, val subjects: List<WikiSubject>, val header: WikiImage? = null, val changelog: WikiChangelog? = null, val recipes: WikiRecipes? = null, val tags: List<ResourceLocation> = listOf()) : JsonSerialisable {
    ...
    override fun toJson() = JsonObject().also {
        ...
        it.addOrNull("changelog", changelog?.getScoped(this))
